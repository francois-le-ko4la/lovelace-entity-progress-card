...
// Builds dist/entity-progress-card.js from the src/ module split instead of
// the entity-progress-card.js monolith (see scripts/build.js) - lets the two
// pipelines be compared while src/ is not yet wired in as the shipped source.
//
...
// feature types and prints the console banner (mirrors the tail of the
// monolith).
//
// esbuild bundles the ES modules into one script first (import/export
// resolved away), then the same CSS-in-JS resolve+minify pass used for the
// monolith build runs over the bundled source, then a final esbuild minify
// pass produces the shipped file.
const fs = require('fs');
