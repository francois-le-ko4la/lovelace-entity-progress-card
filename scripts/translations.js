...
//
// Shipped shape: TRANSLATION_KEYS (dotted leaf paths, stated once) +
// TRANSLATIONS_FLAT (per-language value arrays, indexed on that same list).
// A value identical to English's own is stored as the sentinel 0 instead of
// repeating the string - both key names and cross-language duplicate text
// used to be baked into every one of the 39 language trees. Every other
// command in this file still works with the nested {lang: {a: {b: 'x'}}}
// shape via readJsTranslations()/writeJsTranslations() - only these two
// functions know about the flat on-disk representation.

const JS_BLOCK_RE = /const TRANSLATION_KEYS = (\[[\s\S]*?\n\]);\nconst TRANSLATIONS_FLAT = (\{[\s\S]*?\n\});/;

const readJsTranslations = () => {
  const src = fs.readFileSync(JS_FILE, 'utf8');
  const blockMatch = src.match(JS_BLOCK_RE);
  if (!blockMatch) die('TRANSLATION_KEYS/TRANSLATIONS_FLAT block not found in JS file');
  // The block is generated by this very tool → safe to evaluate in a sandbox.
  const keys = vm.runInNewContext(`(${blockMatch[1]})`);
  const flat = vm.runInNewContext(`(${blockMatch[2]})`);
  const tree = {};
  for (const lang of Object.keys(flat)) {
    const langTree = {};
    keys.forEach((key, i) => setPath(langTree, key, flat[lang][i] === 0 ? flat.en[i] : flat[lang][i]));
    tree[lang] = langTree;
  }
  return tree;
};
...
const writeJsTranslations = (translations) => {
  ...
  const content = [
    ...
    `const TRANSLATIONS_FLAT = ${formatJS(flat)};`,
